...
import (
	...
	cfg "github.com/khoa-nguyendang/ddd-clean-architect-go-template/core/specifications"
...
func GetClient(ctx context.Context, config cfg.MongodbConfig) (*mongo.Client, error) {
	clientOpts := options.Client().ApplyURI(config.MongodbHost)
...
func GetClientSRV(ctx context.Context, config cfg.MongodbConfig) (*mongo.Client, error) {
	uri := fmt.Sprintf("mongodb+srv://%s:%s@%s:%s/%s?retryWrites=true&w=majority", config.MongodbUser, config.MongodbPassword, config.MongodbHost, config.MongodbPort, config.MongodbDbname)
...
func GetOne(client *mongo.Client, config cfg.MongodbConfig, mgCollection, mongoId string, output interface{}) error {
	...
	collection := client.Database(config.MongodbDbname).Collection(mgCollection)
...
func GetOneByFilter(client *mongo.Client, config cfg.MongodbConfig, mgCollection string, filter interface{}, output interface{}) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	var err error
	if client == nil {
...
func Insert(client *mongo.Client, config cfg.MongodbConfig, mgCollection string, v interface{}) (interface{}, error) {
	if v == nil {
		return "", errors.New("model is empty")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
...
func InsertJson(client *mongo.Client, config cfg.MongodbConfig, mgCollection, jsonString string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	var err error
	if client == nil {
...
func UpdateOne(client *mongo.Client, config cfg.MongodbConfig, mgCollection string, filter interface{}, model interface{}) (int64, error) {
	if model == nil {
...
func CloudUpdateOne(config cfg.MongodbConfig, collectionName string, filter interface{}, update interface{}) (interface{}, error) {
	if update == nil {
		return "", errors.New("model is empty")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	...
	collection := client.Database(config.MongodbDbname).Collection(collectionName)
...
func CloudCollectionCount(config cfg.MongodbConfig, collectionName string, filter interface{}) (int64, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	client, err := GetClientSRV(ctx, config)
	defer func() {
...
func Update(client *mongo.Client, config cfg.MongodbConfig, mgCollection, id string, v interface{}) (int64, error) {
	if v == nil {
		return 0, errors.New("model is empty")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
...
func Delete(client *mongo.Client, config cfg.MongodbConfig, mgCollection, id string) (int64, error) {
...
func DeleteMany(client *mongo.Client, config cfg.MongodbConfig, mgCollection string, filter bson.M) (int64, error) {
