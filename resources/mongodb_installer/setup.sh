...
mongo --host ${MONGODB1} --port 27017  <<EOF
use admin;
db.createUser( { user: 'admin_User', pwd: 'admin_Password@1234', roles: [ { role: 'readWrite', db: 'admin' }]});
use faiss;
db.createUser( { user: 'admin_User', pwd: 'admin_Password@1234', roles: [ { role: 'readWrite', db: 'faiss' }]});
EOF