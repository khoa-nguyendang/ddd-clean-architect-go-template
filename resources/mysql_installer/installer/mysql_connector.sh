...
mysql --host $GOTEMPLATE_MYSQL_HOST -uroot -p$MYSQL_MASTER_PASSWORD  -AN -e "CREATE USER '$MYSQL_REPLICATION_USER'@'%' IDENTIFIED WITH mysql_native_password BY  '$MYSQL_REPLICATION_PASSWORD';"
mysql --host $GOTEMPLATE_MYSQL_HOST -uroot -p$MYSQL_MASTER_PASSWORD  -AN -e "GRANT REPLICATION SLAVE ON *.* TO '$MYSQL_REPLICATION_USER'@'%';"
mysql --host $GOTEMPLATE_MYSQL_HOST -uroot -p$MYSQL_MASTER_PASSWORD  -AN -e 'flush privileges;'


echo "* Set MySQL01 as master on MySQL02"

MYSQL01_Position=$(eval "mysql --host $GOTEMPLATE_MYSQL_HOST -uroot -p$MYSQL_MASTER_PASSWORD  -e 'show master status \G' | grep Position | sed -n -e 's/^.*: //p'")
MYSQL01_File=$(eval "mysql --host $GOTEMPLATE_MYSQL_HOST -uroot -p$MYSQL_MASTER_PASSWORD  -e 'show master status \G'     | grep File     | sed -n -e 's/^.*: //p'")
MASTER_IP=$(eval "getent hosts $GOTEMPLATE_MYSQL_HOST|awk '{print \$1}'")
echo $MASTER_IP
mysql --host $MYSQL_REP1_HOST -uroot -p$MYSQL_SLAVE_PASSWORD  -AN -e "CHANGE MASTER TO master_host='$GOTEMPLATE_MYSQL_HOST', master_port=3306, \
...
mysql --host $GOTEMPLATE_MYSQL_HOST -uroot -p$MYSQL_MASTER_PASSWORD  -AN -e 'set GLOBAL max_connections=2000';
...
echo
echo MYSQL01_IP       : $GOTEMPLATE_MYSQL_HOST
